...
export default function MissionEdit() {
  ...
  const { data: missionEmployees, isLoading: isEmployeesLoading } = useGetMissionEmployees(id, {
  ...
  useEffect(() => {
    if (mission && missionEmployees && !formReady) {
      const empIds = missionEmployees.map((e) => e.employeeId);
      form.reset({
        title: mission.title,
        needsExpression: mission.needsExpression,
        actionPlan: mission.actionPlan,
        startDate: mission.startDate,
        endDate: mission.endDate,
        destination: mission.destination,
        requiresFuel: mission.requiresFuel ?? false,
        requiresVehicle: mission.requiresVehicle ?? false,
        vehicleCount: mission.vehicleCount ?? 0,
        employeeIds: empIds,
      });
      setFormReady(true);
    }
  }, [mission, missionEmployees, form, formReady]);
  ...
  const onSubmit = (data: MissionFormValues) => {
    ...
    updateMission.mutate({ id, data });
  };

  if (isMissionLoading || isEmployeesLoading || !formReady) {
    return (
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Modifier la Mission #{id}</h1>
          <p className="text-muted-foreground">Mettez à jour les informations de la mission.</p>
        </div>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                                  ...
                                  <div className="flex items-center gap-2 flex-1">
                                    ...
                                    <div className="flex flex-col">
                                      ...
                                      <span className="text-xs text-muted-foreground">
                                        {employee.position} ({employee.matricule})
                                      </span>
                                    </div>
                                  </div>
                                </CommandItem>
                    ...
                    {selectedEmployeeIds.length > 0 && (
                      <div className="mt-4 p-4 border rounded-md bg-muted/20">
                        <h4 className="text-sm font-medium mb-2">
                          Sélection ({selectedEmployeeIds.length})
                        </h4>
                        <div className="flex flex-wrap gap-2">
                          {selectedEmployeeIds.map((empId) => {
                            const emp = employeesData?.data.find((e) => e.id === empId)
                              ?? missionEmployees?.find((e) => e.employeeId === empId);
                            const name = emp
                              ? ("fullName" in emp ? emp.fullName : (emp as { fullName?: string }).fullName ?? `ID: ${empId}`)
                              : `ID: ${empId}`;
                            return (
                              <div
                                key={empId}
                              ...
                              >
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    field.onChange(field.value.filter((v) => v !== empId))
                                  }
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Besoins en véhicules et carburant.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <FormField
                control={form.control}
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4 shadow-sm">
                    ...
                    <div className="space-y-1 leading-none">
                      <FormLabel>Nécessite un ou plusieurs véhicules SOMELEC</FormLabel>
                      <FormDescription>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4 shadow-sm">
                    ...
                    <div className="space-y-1 leading-none">
                      <FormLabel>Nécessite une dotation en carburant</FormLabel>
                      <FormDescription>
          ...
          <div className="flex justify-end gap-4">
            ...
            <Button type="submit" disabled={updateMission.isPending}>
